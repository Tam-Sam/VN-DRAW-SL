(function (ca) {
	...
	function va(b) {
		...
		e = c * c;
		for (c = 0; c < a - 1; c += 2)
			if (((g = S({ x: b[c], y: b[c + 1] }, d)), 0.8 > g / e || 1.2 < g / e)) return !1;
		return !0;
	}
	function ja(b, a) {
		var c = {};
		...
		var d = wa,
			f;
		f = 0 >= a ? 90 : 1 <= a ? -90 : ((2 * xa(ya(E * (1 - 2 * a))) - J) / E) * 180;
		c[d] = f;
		...
		$ ||
			($ = setInterval(function () {
				for (var b = F.items, a = !1, c = 0, d = b.length; c < d; c++)
					1 > b[c].scale && ((b[c].scale += 0.1), 1 < b[c].scale && (b[c].scale = 1), (a = !0));
				A.render();
	...
	var u = Math,
		...
		Ja = (function () {
			...
			function a(a, b) {
				...
				for (n = a.length - 3; p < n; p += 2)
					((c = a[p]), (d = a[p + 1]), (f = a[p + 2]), (s = a[p + 3]), (D += c * s - f * d));
				...
				switch (b.type) {
					...
					case 'MultiPolygon':
						...
						for (f = b.coordinates.length; d < f; d++)
							(y = c({ type: 'Polygon', coordinates: b.coordinates[d] })) && l.push.apply(l, y);
						return l;
					case 'Polygon':
						b = b.coordinates;
						break;
				...
				read: function (a) {
					...
					for (f = a.length; d < f; d++)
						if (((n = a[d]), 'Feature' === n.type && !1 !== ta(n))) {
							...
							if ((q = e.material ? b(e.material) : e.wallColor || e.color)) g.wallColor = q;
							if ((q = e.roofMaterial ? b(e.roofMaterial) : e.roofColor)) g.roofColor = q;
							switch (e.shape) {
								case 'cylinder':
								case 'cone':
								case 'dome':
							...
							g.roofShape && e.roofHeight
								? ((g.roofHeight = e.roofHeight), (g.height = K(0, g.height - g.roofHeight)))
								: (g.roofHeight = 0);
							t = g;
							q = c(n.geometry);
							e = 0;
							for (g = q.length; e < g; e++) {
								...
								if (r.isRotational) {
									for (
										var w = r, u = r.footprint, x = 180, z = -180, v = 0, A = u.length;
										v < A;
										v += 2
									)
		...
		F = {
			...
			getPixelFootprint: function (b) {
				for (var a = new ra(b.length), c, d = 0, f = b.length - 1; d < f; d += 2)
			...
			addRenderItems: function (b, a) {
				for (var c, d, f, e = Ja.read(b), g = 0, h = e.length; g < h; g++)
					((c = e[g]),
						(f = c.id || [c.footprint[0], c.footprint[1], c.height, c.minHeight].join()),
						!this.loadedItems[f] &&
							(d = this.scale(c)) &&
							((d.scale = a ? 0 : 1), this.items.push(d), (this.loadedItems[f] = 1)));
				Ca();
			},
			scale: function (b) {
				var a = {},
				...
				if (
					...
					for (
						...
						((f = U(f, d[k])), (e = K(e, d[k])), (g = U(g, d[k + 1])), (h = K(h, d[k + 1])));
					...
					b.wallColor &&
						...
						((n = n.alpha(C)), (a.altColor = '' + n.lightness(0.8)), (a.wallColor = '' + n));
					b.roofColor && (n = I.parse(b.roofColor)) && (a.roofColor = '' + n.alpha(C));
			...
			update: function () {
				...
				this.resetItems();
				if (!(x < G))
					if (this.isStatic && this._staticData) this.addRenderItems(this._staticData);
					else if (this.src) {
		...
		Z = {
			draw: function (b, a, c, d, f, e, g, h) {
				var k,
					...
					l = [];
				if (c) for (a = 0, k = c.length; a < k; a++) l[a] = this._extrude(b, c[a], d, f, e, g);
				b.fillStyle = h;
				b.beginPath();
				this._ring(b, m);
				...
				for (
					...
					((h.x = a[s] - p),
						...
						(k.x - h.x) * (m.y - h.y) > (m.x - h.x) * (k.y - h.y) &&
							((b.fillStyle = (h.x < k.x && h.y < k.y) || (h.x > k.x && h.y > k.y) ? e : f),
							b.beginPath(),
			...
			shadow: function (b, a, c, d, f) {
				for (
					var e = null, g = { x: 0, y: 0 }, h = { x: 0, y: 0 }, k, m, l = 0, q = a.length - 3;
					l < q;
					l += 2
				)
					((g.x = a[l] - p),
		...
		w = {
			draw: function (b, a, c, d, f, e, g, h, k) {
				...
				(m = this._tangents(a, c, f, d))
					? ((e = P(m[0].y1 - a.y, m[0].x1 - a.x)), (m = P(m[1].y1 - a.y, m[1].x1 - a.x)))
					: ((e = 1.5 * E), (m = 1.5 * E));
				b.fillStyle = g;
				b.beginPath();
				...
				for (var k = { x: 0, y: 0 }, m = 0, l = a.length - 3; m < l; m += 2)
					((d.x = a[m] - p),
						...
						(k.x - d.x) * (c.y - d.y) > (c.x - d.x) * (k.y - d.y) &&
							((b.fillStyle = (d.x < k.x && d.y < k.y) || (d.x > k.x && d.y > k.y) ? g : e),
							b.beginPath(),
			...
			shadow: function (b, a, c, d, f) {
				...
				for (var h = a.length - 3; d < h; d += 2)
					((e.x = a[d] - p),
						...
						(g.x - e.x) * (c.y - e.y) > (c.x - e.x) * (g.y - e.y) && this._triangle(b, e, g, c));
			},
			...
			hitArea: function (b, a, c, d, f, e) {
				...
				for (var k = a.length - 3; e < k; e += 2)
					((d.x = a[e] - p),
						...
						(h.x - d.x) * (c.y - d.y) > (c.x - d.x) * (h.y - d.y) && this._triangle(b, d, h, c));
				b.closePath();
				b.fill();
			}
		},
		r = {
			...
			render: function () {
				...
				if (!(x < G || Q)) {
					...
					m.sort(function (a, b) {
						return (
							a.minHeight - b.minHeight || S(b.center, f) - S(a.center, f) || b.height - a.height
						);
					});
			...
			render: function () {
				...
				if (!(x < G || Q || x > this.maxZoom))
					for (var a, c, d = F.items, f = 0, e = d.length; f < e; f++)
						if (((a = d[f]), !(a.height >= this.maxHeight) && ((c = a.footprint), V(c))))
							switch (
								((b.strokeStyle = a.altColor || aa), (b.fillStyle = a.roofColor || X), a.shape)
							) {
								case 'cylinder':
								case 'cone':
								case 'dome':
		...
		z = {
			...
			render: function () {
				...
				if (!(
					...
					x < G ||
					Q ||
					((a = ja(W + p, fa + n)), (a = Ha(this.date, a.latitude, a.longitude)), 0 >= a.altitude)
				)) {
					c = 1 / ka(a.altitude);
					d = 5 > c ? 0.75 : (1 / c) * 5;
		...
		Y = {
			...
			_render: function () {
				...
				if (!(x < G || Q)) {
					...
					B && v && (this._imageData = this.context.getImageData(0, 0, B, v).data);
				}
			},
