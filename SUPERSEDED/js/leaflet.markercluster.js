...
!(function (e, t, i) {
	((L.MarkerClusterGroup = L.FeatureGroup.extend({
					...
					_ = L.bind(function () {
						for (var d = new Date().getTime(); a > h; h++) {
							...
							if (((t = e[h]), t instanceof L.LayerGroup))
								...
								if (
									!this.hasLayer(t) &&
									(this._addLayer(t, this._maxZoom), t.__parent && 2 === t.__parent.getChildCount())
								) {
					...
					i instanceof L.LayerGroup
						...
						: i.__parent
							? (this._removeLayer(i, !0, !0),
								s.hasLayer(i) && (s.removeLayer(i), i.clusterShow && i.clusterShow()))
							: r.removeLayer(i));
			return (
				this._topClusterLevel._recalculateBounds(),
				this._topClusterLevel._recursivelyAddChildrenToMap(
		...
		clearLayers: function () {
			return (
				this._map ||
					((this._needsClustering = []), delete this._gridClusters, delete this._gridUnclustered),
		...
		eachLayer: function (e, t) {
			...
			for (
				this._topClusterLevel && this._topClusterLevel.getAllChildMarkers(n), i = n.length - 1;
				i >= 0;
				i--
			)
		...
		_extractNonGroupLayers: function (e, t) {
			...
			for (t = t || []; s < n.length; s++)
				((i = n[s]), i instanceof L.LayerGroup ? this._extractNonGroupLayers(i, t) : t.push(i));
			return t;
		},
							...
							for (
								n.contains(a) || (a = null),
									r._isSingleParent() && e + 1 === t
										? (s.removeLayer(r), r._recursivelyAddChildrenToMap(null, t, n))
										: (r.clusterHide(), r._recursivelyAddChildrenToMap(a, t, n)),
									i = h.length - 1;
								i >= 0;
								i--
							)
					...
					(n.addLayer(e),
						t !== e &&
							(t._childCount > 2
								...
								: (this._forceLayout(),
									i._animationStart(),
									i._animationZoomOutSingle(t, this._map.getMaxZoom(), this._map.getZoom()))));
				}
			},
			...
			_recalculateBounds: function () {
				...
				if (0 !== h) {
					for (this._resetBounds(), e = 0; e < s.length; e++)
						((i = s[e]._latlng), this._bounds.extend(i), (o += i.lat), (a += i.lng));
			...
			_recursively: function (e, t, i, n, s) {
				...
				if (t > h)
					for (r = a.length - 1; r >= 0; r--)
						((o = a[r]), e.intersects(o._bounds) && o._recursively(e, t, i, n, s));
		...
		L.MarkerCluster.include({
			...
			_generatePointsCircle: function (e, t) {
				...
				for (a.length = e, i = e - 1; i >= 0; i--)
					((n = this._circleStartAngle + i * o),
						(a[i] = new L.Point(t.x + r * Math.cos(n), t.y + r * Math.sin(n))._round()));
				return a;
			},
			_generatePointsSpiral: function (e, t) {
				...
				for (h.length = e, i = e - 1; i >= 0; i--)
					((a += r / s + 5e-4 * i),
						(h[i] = new L.Point(t.x + s * Math.cos(a), t.y + s * Math.sin(a))._round()),
						(s += o / a));
				return h;
			},
			_noanimationUnspiderfy: function () {
				...
				for (i._ignoreMove = !0, this.setOpacity(1), t = r.length - 1; t >= 0; t--)
					((e = r[t]),
						s.removeLayer(e),
		...
		L.MarkerCluster.include({
			_animationSpiderfy: function (e, t) {
				var n,
					...
					g = m.opacity;
				for (
					g === i && (g = L.MarkerClusterGroup.prototype.options.spiderLegPolylineOptions.opacity),
						f
							? ((m.opacity = 0),
				...
				for (l._forceLayout(), l._animationStart(), n = e.length - 1; n >= 0; n--)
					((h = _.layerPointToLatLng(t[n])),
						(s = e[n]),
